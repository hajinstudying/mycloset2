...
public class PageNavigator {
   ...
   public String getPageNavigator(int totalCount, int listCount, int pagePerBlock,
      ...
      if(totalCount > 0) {   //총 레코드 수가 하나라도 있어야 페이징
    	  ...
    	  /*
    	   * [총 페이지 수]
    	   * - 총 게시물이 101개이고 한 페이지에 10개의 페이지 번호를 노출시켜야 한다고 가정.
         ...
         /*
          * [총 페이지 블럭]
          * - 전체페이지가 11개이고 한페이지에 10개의 페이지번호를 표시한다고 가정하면 총 페이지블럭이 2개가 됨
          * - 11 % 10 != 0 이면 페이지블럭은 2가 된다. [1...10][11...20] 
          */
         
         int totalNumOfBlock = (totalNumOfPage % pagePerBlock == 0) ? totalNumOfPage / pagePerBlock : totalNumOfPage / pagePerBlock + 1;
         
         /*
          * [요청된 페이지가 몇 번째 페이지 블록(그룹)에 속하는지]
         ...
         int currentBlock = (pageNum % pagePerBlock == 0) ? 
               pageNum / pagePerBlock : // 1 페이지 블럭([1...10])
               pageNum / pagePerBlock + 1; // 2 페이지 블럭([11...20])
         
         System.out.println("currentBlock : " + currentBlock);
         
         /*
          * 시작 페이지 번호: (현재페이지 블럭 -1) * 10 + 1 = 1
          */
         int startPage = (currentBlock - 1) * pagePerBlock + 1;

         /*
          * 끝 페이지 번호: 시작페이지 + 10 - 1 = 10
          */
          ...
          * - 실제 페이지 객수가 11인데 계산 공식에 의해서 계산된 끝페이지는 20이다.
         ...
         /*
          * 다음이 페이지가 있는지 표식> >>
          * - 현재 페이지블럭(1) < 전체 페이지 블럭 (2) 표식을 노출시킨다.
          */
         ...
         /*
          * 총 페이지 블럭이 단 한개인 경우 뒤로가기/앞으로가기 표식이 노출 안됨
          */
         ...
         if(pageNum > 1){   // 쿼리스트링에서 & = &amp; 
            sb.append("<a href=\"").append("productList?pageNum=1");
            sb.append("\" title=\"<<\"><<</a>&nbsp;");
         }

         if (isPrev) {   
            int goPrevPage = startPage - pagePerBlock;         
            sb.append("&nbsp;&nbsp;<a href=\"").append("productList?pageNum="+goPrevPage+"");
            sb.append("\" title=\"<\"><</a>");
         } else {
         ...
         for (int i = startPage; i <= endPage; i++) {
            if (i == pageNum) {
               ...
               sb.append("<a href=\"").append("productList?pageNum="+i);
         ...
         if (isNext) {
            ...
            sb.append("<a href=\"").append("productList?pageNum="+goNextPage);
            sb.append("\" title=\">\">></a>");
         } else {
            
         }
         if(totalNumOfPage > pageNum){
            sb.append("&nbsp;&nbsp;<a href=\"").append("productList?pageNum="+totalNumOfPage);
            sb.append("\" title=\">>\">>></a>");
         }
      }
